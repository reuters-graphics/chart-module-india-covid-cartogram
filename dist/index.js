  ...
  _createClass(MyChartModule, [{
    ...
    value: function getStatesArray(data, props) {
      ...
      */
      ...
      props.uniformMax = max(statesArray, function (d) {
        return d.max;
    ...
    function draw() {
      ...
      var xGridDom = range(1, props.cols + 1);
      var yGridDom = range(1, props.rows + 1); // Format the data (See function for documentation)

      var statesArray = this.getStatesArray(data, props);
      var xGridScale = band().rangeRound([0, wh * props.cols]).padding(0.05).domain(xGridDom);
      var yGridScale = band().rangeRound([0, wh * props.rows]).padding(0.05).domain(yGridDom); // Inner x scale for drawing lines along the date axis.

      var xScale = linear().range([0, wh - innerMargin.left - innerMargin.right]).domain([0, data.series.length - 1]); // inverse scale for getting tooltip dates.

      var inverseX = linear().domain([0, wh - innerMargin.left - innerMargin.right]); // works with inverse scale for getting tooltip dates.

      var scaleXTime = scaleTime().domain([new Date(data.series[0]), new Date(data.series[data.series.length - 1])]).range([0, wh - innerMargin.left - innerMargin.right]);
      ...
      function makeLine(datum) {
        ...
        datum.yScale = linear().range([wh - innerMargin.top - innerMargin.bottom, 0]).domain([0, yMax]);
        var line = d3Line().defined(function (d) {
      ...
      touchBox.on('mouseover mousemove touchenter touchstart touchmove', function (d, i, nodes) {
        ...
        var mx = mouse(parent)[0] - innerMargin.left;
        ...
        select(parent).appendSelect('circle.tooltip').attr('r', 3).attr('cx', x + innerMargin.left).attr('cy', y + innerMargin.top).style('fill', 'white').style('stroke', 'white').style('stroke-width', 1);
        select(parent).appendSelect('text.tooltip.datum').attr('x', x).attr('y', y < d.yScale.range()[1] / 2 ? y + 20 : y - 7).style('text-align', 'center').style('text-anchor', x > wh / 2 ? 'end' : 'start').text(numberFormat(round(datum[props.lineVar], 0)));
        select(parent).appendSelect('text.tooltip.date').attr('x', x).attr('y', d.yScale.range()[0] + 5 + innerMargin.bottom).text(dateFormat(date)).style('text-anchor', x > wh / 2 ? 'end' : 'start');
      });
