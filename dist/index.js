...
var MyChartModule = /*#__PURE__*/function () {
  ...
  _createClass(MyChartModule, [{
      ...
      */
      ...
      Object.keys(data.states).forEach(function (key, i) {
        ...
        data.states[key].reported[props.cat].forEach(function (d, index) {
          ...
          var obj = {
            val: d,
            avg7day: lastWeek.length === 7 ? d3Array.mean(lastWeek) : null
          };
        ...
        var obj = {
          ...
          max: d3Array.max(series, function (d) {
      ...
      props.uniformMax = d3Array.max(statesArray, function (d) {
        return d.max;
    ...
    function draw() {
      ...
      var xGridDom = d3Array.range(1, props.cols + 1);
      var yGridDom = d3Array.range(1, props.rows + 1); // Format the data (See function for documentation)

      var statesArray = this.getStatesArray(data, props);
      var xGridScale = d3Scale.scaleBand().rangeRound([0, wh * props.cols]).padding(0.05).domain(xGridDom);
      var yGridScale = d3Scale.scaleBand().rangeRound([0, wh * props.rows]).padding(0.05).domain(yGridDom); // Inner x scale for drawing lines along the date axis.

      var xScale = d3Scale.scaleLinear().range([0, wh - innerMargin.left - innerMargin.right]).domain([0, data.series.length - 1]); // inverse scale for getting tooltip dates.

      var inverseX = d3Scale.scaleLinear().domain([0, wh - innerMargin.left - innerMargin.right]); // works with inverse scale for getting tooltip dates.

      var scaleXTime = d3Scale.scaleTime().domain([new Date(data.series[0]), new Date(data.series[data.series.length - 1])]).range([0, wh - innerMargin.left - innerMargin.right]);
