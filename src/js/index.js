...
class MyChartModule {
  ...
  getStatesArray(data, props) {
    ...
    Object.keys(data.states).forEach((key, i) => {
      ...
      data.states[key].reported[props.cat].forEach((d, index) => {
        ...
        let lastWeek = data.states[key].reported[props.cat].slice(index - 6, index + 1)
          .filter(d=> d != null)
  ...
  draw() {
    ...
    const xGridScale = d3.scaleBand()
      .rangeRound([0, wh * props.cols])
      .padding(.05)
      .domain(xGridDom);

    const yGridScale = d3.scaleBand()
    ...
    const scaleXTime = d3.scaleTime()
      .domain([new Date(data.series[0]), new Date(data.series[data.series.length - 1])])
      .range([0, wh - innerMargin.left - innerMargin.right]);
    ...
    let stateGroup = plot.selectAll(".state-g")
      .data(statesArray)
      .join('g')
      .attr("class", "state-g")
      .attr("transform", d => {
    ...
    this.selection().selectAll('div.state-name')
    ...
    let statePlot = stateGroup.appendSelect('g.state-plot')
      .attr('transform', `translate(${innerMargin.left},${innerMargin.top})`);

    statePlot.appendSelect('path.line')
    ...
    const touchBox = stateGroup.appendSelect('g.dummy-container')
      .appendSelect('rect')
      .attr('height', wh)
      .attr('width', wh)
    ...
    touchBox.on('mouseover mousemove touchenter touchstart touchmove', (event, d) => {

      if (!event) return;
      this.selection().selectAll('.tooltip').remove();
      const parent = event.srcElement.parentNode;
      let mx = event.pageX - parent.getBoundingClientRect().left - innerMargin.left;
