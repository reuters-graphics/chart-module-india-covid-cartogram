    ...
    if (props.parameter === 'cases' || props.parameter === 'deaths') {
      ...
      for (var key of Object.keys(data.states)) {
        ...
        data.states[key][props.parameter].reverse().forEach(function (d, i) {
          data.states[key].avg.push(
            d3.mean(
              data.states[key][props.parameter].slice(i, i + props.avg_days),
              (d) => (d < 0 ? 0 : d)
            )
          );
        });
        ...
        data.states[key].sortOrder =
          data.states[key].percentOfPeak[props.parameter];
      }
      ...
      const transition = d3.transition().duration(250);
      ...
      const inverseX = d3.scaleLinear().domain([0, smallW]);

      const scaleY = d3.scaleLinear();
      if (props.uniformScale) {
        scaleY
          .domain([0, max])
          .range([smallH * 0.9 - 5, smallH * 0.25 + props.paddingTitle]);
      } else {
        scaleY
          .domain([0, 1])
          .range([smallH * 0.9 - 5, smallH * 0.25 + props.paddingTitle]);
      ...
      const statesG = g.selectAll('.state').data(props.stAbbr, (d) => d);
      ...
      statesG
        ...
        .attr('d', (d) => {
          if (props.uniformScale) {
            ...
            return line(
              data.states[d].avg.map((e) => e / d3.max(data.states[d].avg))
            );
          }
        });
      ...
      touchBox.on(
        ...
        (d, i, nodes) => {
          ...
          index =
            index < 0
              ? 0
              : index >= data.states[d].avg.length
              ? data.states[d].avg.length - 1
              : index;
          const datum = data.states[d].avg[index];
          const datumY = props.uniformScale
            ? datum
            : datum / d3.max(data.states[d].avg);
      ...
      const stateNamesContainer = this.selection().appendSelect(
        'div.name-container'
      );

      const stateNames = stateNamesContainer
      ...
      stateNames
        ...
        .classed(
          'bold',
          (d) => data.states[d].percentOfPeak[props.parameter] >= 0.9
        )
        ...
        .html(
          (d) =>
            `${getArrow(data.states[d].avg)} <p>${data.states[d].stateAP}</p>`
        );
